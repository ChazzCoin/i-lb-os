...
New EngineAnimateControlPill (Components.swift): Record toggle (start/stopRecording), Restart, and Play/Pause wired to BEO.playAnimationRecording()/stopAnimationRecording() for the selected playbackRecordingId — reconnects the orphaned engine play path. Record removed from the Plan pill (showRecord:false) so record/playback only appear in Animate (the explicit ask). Locks the board during playback via onChange(isPlayingAnimation). Verified rendering on sim; full play-through is interaction-level (XCUITest / on-device).
