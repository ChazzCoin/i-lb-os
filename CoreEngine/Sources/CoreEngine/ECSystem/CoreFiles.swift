...
public func getDataFromFile(title: String, fileURL: URL, withData: @escaping (Data) -> Void) {
...
public func accessDataFromFile(fileUrl: URL, dataAccess: @escaping (Data) -> Void) {
    if let data = try? Data(contentsOf: fileUrl) { dataAccess(data) }
}

// File Bookmarking
public func bookmarkFile(fileUrl: URL, keyName: String = "FileBookmark") {
    // Security Scoping
    do {
...
public func accessBookmarkedFile(keyName: String = "FileBookmark", fileAccess: @escaping (URL) -> Void) {
