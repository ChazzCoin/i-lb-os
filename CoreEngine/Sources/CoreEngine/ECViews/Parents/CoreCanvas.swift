...
public struct CoreCanvasEngine<G: View, C: View>: View {
    public var global: (GeometryProxy, GlobalPositioningSystem) -> G
    public var canvas: (CanvasGPS) -> C
    public init(@ViewBuilder global: @escaping (GeometryProxy, GlobalPositioningSystem) -> G, @ViewBuilder canvas: @escaping (CanvasGPS) -> C) {
        self.global = global
        self.canvas = canvas
    }
...
//    @StateObject public var managedWindowsObject: NavWindowController = NavWindowController()
    ...
    public let initialWidth: CGFloat = 6000
    public let initialHeight: CGFloat = 6000
    ...
    public var body: some View {
        ...
        if !masterResetCanvas {
            ...
            GlobalPositioningReader(coordinateSpace: CoreNameSpace.local) { geo, gps in
                global(geo,gps)
            }
            .zIndex(3.0)
